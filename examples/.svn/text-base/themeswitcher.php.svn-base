...
if (!empty($_GET))
  $LUMINOUS_THEME = $_GET['theme_switcher'];
?>
<!DOCTYPE HTML>
<html>
  <head>
    ...
    $(document).ready(function() {
      $('#theme_switcher').change(function(){
        var theme = $(this).val(),
            ...
            echo "'" . wordwrap(join("', '", luminous_get_themes())) . "'"; 
            ?>],
  ...
  $default_theme = $LUMINOUS_THEME;
  if (!preg_match('/\.css$/', $default_theme))
    $default_theme .= '.css';
  foreach(luminous_get_themes() as $theme)
  {
  ...
  <p>  
  <?php echo luminous_file('php', __FILE__, $use_cache); ?> 
  </body>
</html>
