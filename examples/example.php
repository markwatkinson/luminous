...
<html>
    ...
    <body>
        ...
        <p> Inspect the source code to see what's going on. Look at both the PHP code and the HTML markup. </p>
        <p>
            <?php
            if (!$useCache) {
                echo "Caching is <strong>disabled</strong>, click <a href='example.php'>here</a> to enable it";
            } else {
                echo "Caching is <strong>enabled</strong>. If you are seeing errors, you will need to make the directory: " . realpath(__DIR__  . "/../") . "/cache/, and make it writable to your server if you intend to use the caching system. Click <a href='example.php?nocache'>here</a> to view this page with caching disabled";
            }
            ?>
        </p>
        <?= Luminous::highlight('cpp', <<<EOF
#include <stdio.h>
int main()
{
  printf("hello, world");
...
EOF
        ...
        <p>
            You can also set specific runtime options in the highlight call (here we set 'max-height' = 250), which
            will be forgotten at the next highlight.
            <?= Luminous::highlight('php', <<<EOF
<?php
/**
...
EOF
            , array('cache' => $useCache, 'maxHeight' => '250')); ?>
        <p>
            See:
            <?= Luminous::highlight('php', <<<EOF
<?php
