...
class LuminousJavaScriptScanner extends LuminousECMAScriptScanner {
  ...
  public static function guess_language($src) {
    ...
    // value more than ~40%
    $p = 0;
    // try to avoid positives on JavaScript inside HTML, but let's strip
    // comments first
    $src_ = preg_replace('% //.* | /\\* .*? \\*/ %x', '', $src);
//    if ($src_ !== null && preg_match('/<script\\b/', $src)) 
  //    return 0;
    // var x = 
    // not amazingly distinct, but something to hang onto
