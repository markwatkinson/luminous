...
class Filters
{
     ...
     * @internal
     */
    private static function doxygenArgLength($command)
    {
     ...
     * @internal
     */
    private static function doxygenizeCb($matches)
    {
        $lead = $matches[1];
        $tagChar = $matches[2];
        $tag = $matches[3];
        ...
        if (isset($l_[0]) && $l_[0] === '{') {
            ...
            return "$lead<DOCTAG>$tagChar$tag</DOCTAG>$line";
        } else {
            $len = self::doxygenArgLength($tag);
        }
        ...
        if ($len === 0) {
            ...
            $l = explode(' ', $line);
            $start = "$lead<DOCTAG>$tagChar$tag</DOCTAG><DOCPROPERTY>";
     ...
     * @internal
     */
    public static function doxygenize($token)
    {
        $token = Utils::escapeToken($token);
        $token[1] = preg_replace_callback(
            "/(^(?>[\/\*#\s\{]*))([\@\\\])([\\w]*)(\}|[ \t]+.*?)?$/m",
            array('Luminous\\Core\\Filters', 'doxygenizeCb'),
            $token[1]
        );
        return $token;
    }
     ...
     * doxygenize() directly.
     */
    public static function genericDocComment($token)
    {
     ...
     * "HACK", "BUG"
     */
    public static function commentNote($token)
    {
        $token = Utils::escapeToken($token);
        $token[1] = preg_replace(
            '/\\b(?:NOTE|XXX|FIXME|TODO|HACK|BUG):?/',
            '<COMMENT_NOTE>$0</COMMENT_NOTE>',
            $token[1]
        );
        return $token;
    }
     ...
     */
    public static function pcre($token, $delimited = true)
    {
        ...
        if ($delimited) {
            ...
            if (preg_match("/[[:alpha:]]+$/", $str, $matches)) {
                ...
                $str = preg_replace(
                    "/((?<!\A)[^[:alnum:]\s<>])([[:alpha:]]+)$/",
                    "<DELIMITER>$1</DELIMITER><KEYWORD>$2</KEYWORD>",
                    $str
                );
            } else {
     ...
     */
    public static function upperToConstant($token)
    {
     ...
     */
    public static function cleanIdent($token)
    {
        if ($token[0] === 'IDENT') {
            $token[0] = null;
        }
