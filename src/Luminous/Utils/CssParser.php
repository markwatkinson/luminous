...
class CssParser
{
  // source: http://www.w3schools.com/css/css_colornames.asp
    private static $hexcolors = array(
        'aliceblue'            => '#f0f8ff',
        'antiquewhite'         => '#faebd7',
        'aqua'                 => '#00ffff',
        'aquamarine'           => '#7fffd4',
    ...
    private static function formatPropertyValue($prop, $value)
    {
        // drop !important
        ...
        switch ($prop) {
            case 'color':
                if (isset(static::$hexcolors[$value])) {
                    $value = static::$hexcolors[$value];
                }
                break;
    ...
    private static function formatCssArray($css)
    {
        $css_ = array();
        ...
        foreach ($css as $selector => $rules) {
            ...
            foreach ($rules as $prop => $value) {
                list($prop, $value) = self::formatPropertyValue($prop, $value);
                $rules_[$prop] = $value;
            }
     ...
     */
    public function value($ruleName, $property, $default = null)
    {
        if (isset($this->data[$ruleName][$property])) {
            return $this->data[$ruleName][$property];
        } else {
            return $default;
        }
    }
