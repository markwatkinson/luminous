...
class JavaScriptScanner extends EcmaScriptScanner
{
    ...
    public static function guessLanguage($src, $info)
    {
        ...
        if (preg_match('/var\s++\w++\s*+=/', $src)) {
            $p += 0.05;
        }
        // $, jquery
        if (preg_match('/\\b$\\( | jQuery/x', $src)) {
            $p += 0.20;
        }
        // typeof x == undefined
        ...
        if (strpos($src, 'document.') !== false) {
            $p += 0.10;
        }
        if (strpos($src, 'Math.') !== false) {
            $p += 0.05;
        }
        // Anonymous functions
