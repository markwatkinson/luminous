...
class BashScanner extends Scanner
{
    ...
    public function init()
    {
        ...
        $this->addFilter('str-filter', 'STRING', array($this, 'stringFilter'));
    }
    ...
    public function main()
    {
        ...
        while (!$this->eos()) {
            ...
            if ($this->scan('/(\\$?)(\({2})/')) {
                $dollar = $this->matchGroup(1);
                $this->record($this->match(), $dollar? 'KEYWORD' : null);
                if ($this->scanUntil('/\){2}/') !== null) {
            ...
            if ($this->scan('/\\$([{(])/')) {
                ...
                $stack[] = array($this->matchGroup(1), true);
            } elseif ($c === '[') {
                ...
                $this->record($this->get(), 'KEYWORD');
            } elseif ($this->scan('/\$( [_a-zA-Z]\w* | [\d\#*@\-!_\\?\\$])/xm')) {
                ...
                $delimRegex = "/^(" . (($m[1] === '<<-') ? '\s*' : '') . ')(' . preg_quote($m[4], '/') . ')\\b/m';
                $heredoc = $this->scanUntil($delimRegex);
                if ($heredoc === null) {
                    $heredoc = $this->rest();
                    $this->terminate();
                }
                $this->record($heredoc, 'HEREDOC');
                if ($this->scan($delimRegex) !== null) {
                    $g = $this->matchGroups();
                    if ($g[1] !== '') {
            ...
            } elseif ($this->scan('/(<<-?\s*)(\w+)/') || $this->scan('/\\$?"/')) {
                ...
                while (1) {
                    list($index, $matches) = $this->getNext($searches);
                    ...
                    if ($hit === '"') {
                        ...
                        $this->record(substr($this->string(), $pos, $index_ - $pos), $type);
                        $child = new BashScanner($this->string());
                        $child->pos($index_);
    ...
    {
        $p = 0.0;
        if (preg_match('%\\b (?:bash|csh|ksh|zsh|sh) \\b%x', $info['shebang'])) {
            return 1.0;
        }

        // strange conditional syntax -- if [ -z ...  ]
        if (preg_match('/ (if|while) \s++ \\[\s++-\w/x', $src)) {
            $p += 0.10;
        }
        ...
        if (strpos($src, 'case') < strpos($src, 'esac')) {
            $p += 0.1;
        }
